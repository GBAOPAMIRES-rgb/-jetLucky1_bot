...
async function run(){
  ...
  const terms=[
    ...
    "sessionId","accessToken","Authorization","authorization",
    "ssid","SS_ID","token:"
  ];
  for(const url of urls){
    try{
      ...
      for(const term of terms){
        let p=0,n=0;
        while((p=js.indexOf(term,p))>=0&&n<10){
          hits.push({
            term,
            snippet:clean(js.slice(Math.max(0,p-1200),Math.min(js.length,p+2200)))
          });
          p+=term.length;n++;
        }
      }
      console.log("Lucky Jet socket usage probe",JSON.stringify({
        url,http_status:r.status,bytes:js.length,hits:hits.slice(0,30)
      }));
    }catch(e){
      console.log("Lucky Jet socket usage probe",JSON.stringify({
        url,ok:false,error:String(e.message||e)
      }));
    }
  }
}
