...
async function probeLuckyJetMainAuthCodeAtStartup(){
  const urls=[
    ...
    "https://1wmljx.life/resources/v1/app/assets/plugin-B7K2SK9M.js",
    "https://1wmljx.life/resources/v1/app/assets/socket-io-adapter-DSFgpOt0.js"
  ];
  const terms=["/user/token","/user/auth","fetchOverride","crash-gateway","websocket/lifecycle","lucky-jet","1win_lucky","user/auth","user/token"];
  const clean=(s)=>String(s||"").replace(/[A-Za-z0-9_-]{40,}/g,"<redacted-long>");
  for(const url of urls){
    try{
      const rr=await fetch(url,{headers:{
        "Accept":"*/*","Referer":"https://1wmljx.life/","Origin":"https://1wmljx.life",
        "User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/152 Safari/537.36"
      }});
      ...
      for(const term of terms){
        let pos=0,count=0;
        while((pos=js.indexOf(term,pos))>=0&&count<8){
          hits.push({term,snippet:clean(js.slice(Math.max(0,pos-500),Math.min(js.length,pos+900)))});
          pos+=term.length;count++;
        }
      }
      console.log("Lucky Jet main auth code probe",JSON.stringify({url,http_status:rr.status,bytes:js.length,hits:hits.slice(0,40)}));
    }catch(e){
      console.log("Lucky Jet main auth code probe",JSON.stringify({url,ok:false,error:String(e.message||e)}));
    }
  }
}
...
async function probeLuckyJetStateAtStartup(){
  const targets=[
    ...
    {host:"https://1wmljx.life",referer:"https://1wmljx.life/"}
  ];
  const safe=(v,depth=0)=>{
    ...
    if(typeof v==="object")return Object.fromEntries(Object.entries(v).slice(0,60).map(([k,x])=>[k,safe(x,depth+1)]));
    return v;
  };
  for(const t of targets){
    try{
      const rr=await fetch(t.host+"/state",{headers:{
        "Accept":"application/json",
        "Origin":t.host,
        "Referer":t.referer,
        "User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/152 Safari/537.36"
      ...
      console.log("Lucky Jet state probe",JSON.stringify({
        host:t.host,http_status:rr.status,ok:rr.ok,bytes:raw.length,
        response_keys:parsed&&typeof parsed==="object"?Object.keys(parsed).slice(0,80):[],
        safe_response:parsed?safe(parsed):null,
        prefix:parsed?null:raw.slice(0,500)
      }));
    }catch(e){
      console.log("Lucky Jet state probe",JSON.stringify({host:t.host,ok:false,error:String(e.message||e)}));
    }
  }
}
...
async function probeLuckyJetClientBundleAtStartup(){
  ...
  try{
    ...
    const pageScripts=[];
    let jsPos=0;
    while((jsPos=html.indexOf(".js",jsPos))>=0&&pageScripts.length<80){
      let s=jsPos;
      while(s>0&&!["\"","'"," ","\\n","\\r","<",">"].includes(html[s-1]))s--;
      const token=html.slice(s,jsPos+3);
      if(token.includes("/resources/")){pageScripts.push(token);addUrl(token);}
      jsPos+=3;
    }
...
async function probeLuckyJetSocketClientConstructionAtStartup(){
  ...
  try{
    ...
    console.log("Lucky Jet Socket.IO construction probe",JSON.stringify({status:rr.status,bytes:d.length,hits:hits.slice(0,20)}));
  }catch(e){
...
async function probeLuckyJetAuthCookieNamesAtStartup(){
  ...
  const terms=["/user/auth","/user/token","SS_ID","ssid","auth_token","access_token","Authorization","Bearer","Set-Cookie","set-cookie","cookie:"];
  for(const url of urls){
    try{
      const rr=await fetch(url,{headers:{Accept:"*/*",Origin:"https://1wmljx.life",Referer:"https://1wmljx.life/","User-Agent":"Mozilla/5.0"}});
      const d=await rr.text();
      ...
      for(const term of terms){
        let p=0,n=0;
        while((p=d.indexOf(term,p))>=0&&n<8){
          let snippet=d.slice(Math.max(0,p-700),Math.min(d.length,p+1200));
          snippet=snippet.replace(/[A-Za-z0-9_-]{40,}/g,"<redacted-long>");
          snippet=snippet.replace(/(Cookie\\s*[:=]\\s*["']?)[^"']+/gi,"$1<redacted>");
          hits.push({term,snippet});
          p+=term.length;n++;
        }
      }
      const defs=[];
      for(const name of ["h0","d0","Gt","AR"]){
        const re=new RegExp("(?:const|let|var|function)\\s+"+name+"\\s*=|"+name+"=\\(\\)=>");
        const m=re.exec(d);
        if(m)defs.push({name,index:m.index,snippet:d.slice(Math.max(0,m.index-300),Math.min(d.length,m.index+1800)).replace(/[A-Za-z0-9_-]{40,}/g,"<redacted-long>")});
      }
      console.log("Lucky Jet targeted auth-cookie probe",JSON.stringify({url,status:rr.status,bytes:d.length,definitions:defs,hits:hits.slice(0,50)}));
    }catch(e){
      console.log("Lucky Jet targeted auth-cookie probe",JSON.stringify({url,ok:false,error:String(e.message||e)}));
    }
  }
}
...
function probeLuckyJetLoggerAuthHelperAtStartup(){
  ...
  https.get(url,{headers:{Accept:"*/*",Origin:"https://1wmljx.life",Referer:"https://1wmljx.life/", "User-Agent":"Mozilla/5.0"}},res=>{
    let d="";res.on("data",x=>d+=x);res.on("end",()=>{
      const terms=["JI=({","headers:()=>","headers:","authorization:","Authorization","Bearer ","accessToken","access_token","authToken","X-Auth","X-Authorization","credentials:","credentials","fetch(","/user/","token","session","cookie","X-Origin","X-Unique-Key"];
      const hits=[];
...
async function probeLuckyJetOfficialHeadersAtStartup(){
  ...
  try{
    const rr=await fetch(url,{headers:{Accept:"*/*",Origin:"https://1wmljx.life",Referer:"https://1wmljx.life/","User-Agent":"Mozilla/5.0"}});
    const d=await rr.text();
    const terms=["ot.headers=()=>","headers=()=>","headers(){","headers:()=>","function headers","const headers","ot.headers","JI=({","Kw=()","authorization","Authorization","Bearer","cookie","credentials"];
    const hits=[];
    for(const term of terms){
      let p=0,n=0;
      while((p=d.indexOf(term,p))>=0&&n<5){
        let snippet=d.slice(Math.max(0,p-900),Math.min(d.length,p+1800));
        snippet=snippet.replace(/[A-Za-z0-9_-]{40,}/g,"<redacted-long>");
    ...
    console.log("Lucky Jet official headers probe",JSON.stringify({status:rr.status,bytes:d.length,hits:hits.slice(0,40)}));
  }catch(e){
    console.log("Lucky Jet official headers probe",JSON.stringify({ok:false,error:String(e.message||e)}));
  }
}
