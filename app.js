...
async function adminScreen(type){
  ...
  if(type==="diag"){
    ...
    setScreen("Диагностика",center("🩺 Диагностика","Пошаговая read-only проверка реального источника Lucky Jet.",
      ...
      '<button type="button" class="data-row diag-row bridge-token-row" id="getBridgeToken" onclick="window.requestBridgeToken(); return false;"><span class="bridge-token-icon">🔑</span><div><b>Получить временный bridge-токен</b><small>Сгенерировать токен для browser bridge</small></div><em>›</em></button>'+
      '</div>'+
