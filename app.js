...
async function adminScreen(type){
  ...
  if(type==="diag"){
    ...
    $("checkLuckyJetGateway").onclick=async()=>{const b=$("checkLuckyJetGateway"),m=$("luckyJetGatewayMsg");b.disabled=true;m.textContent="Проверяем WebSocket-шлюз…";const rr=await api("/api/luckyjet-gateway-test");b.disabled=false;m.textContent=rr.ok&&rr.connected?"✅ WebSocket-шлюз принимает соединение.":"❌ Шлюз недоступен: "+(rr.message||rr.error||"неизвестная ошибка");};    $("checkLuckyJetProtocol").onclick=async()=>{const b=$("checkLuckyJetProtocol"),m=$("luckyJetProtocolMsg");b.disabled=true;m.textContent="Проверяем авторизацию и канал lucky-jet-94…";const rr=await api("/api/luckyjet-protocol-test");b.disabled=false;if(rr.configured===false){m.textContent="⚠️ "+(rr.message||rr.error);return}if(rr.authenticated||rr.subscribed||rr.publications){m.textContent="✅ Поток отвечает: auth="+(rr.authenticated?"OK":"нет")+" • subscribe="+(rr.subscribed?"OK":"нет")+" • публикаций="+Number(rr.publications||0)+(rr.latest_coefficient!==null?" • коэффициент="+rr.latest_coefficient:"")+(rr.latest_next_coefficient!==null?" • next="+rr.latest_next_coefficient:"")+(rr.latest_round_id?" • roundId получен":"");}else m.textContent="❌ Авторизация/подписка не подтверждены: "+(rr.message||rr.error||"нет данных");};
