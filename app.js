...
function renderRoleUI(){
 ...
 $("bottomNav").innerHTML=role==="owner"?nav("⌂","Главная","home")+nav("🚀","Сигналы","signals")+nav("📊","Аналитика","analytics")+nav("👥","Пользователи","users")+nav("☰","Ещё","more"):nav("⌂","Главная","home")+nav("🚀","Сигналы","signals")+nav("📜","История","history")+nav("◉","Профиль","profile")+nav("☰","Ещё","more");
 bindSections();renderOwnerPanel();
}
function openMore(){
 const ownerItems=role==="owner"?[
  ["📜","История","Подтверждённые данные","history"],["🔐","Управление доступом","Ограничения пользователей","access"],["⚙️","Управление ботом","Пауза и режим","bot"],["🩺","Диагностика","Проверка системы","diag"],["📋","Логи","Статус событий","logs"],["👑","Owner Panel","Информация Owner","owner"]
 ]:[];
 const common=[["◉","Профиль","1win ID и аккаунт","profile"],["💬","Поддержка","Помощь и связь","support"],["⚙","Настройки","Язык и часовой пояс","settings"]];
 const all=ownerItems.concat(common);
 showPanel("Ещё",'<div class="section-menu">'+all.map(x=>card(x[0],x[1],x[2],x[3],false)).join("")+'</div>');
 panel.querySelectorAll("[data-section]").forEach(b=>b.onclick=()=>{panel.classList.add("hidden");const s=b.dataset.section;if(s==="owner"){ownerPanel.classList.remove("hidden");ownerPanel.scrollIntoView({behavior:"smooth",block:"start"});}else{routeSection(s);}});
}
