...
function showPanel(title,html){panel.innerHTML="<div class='panel-head'><h2>"+title+"</h2><button class='quick-link' onclick='document.getElementById("panel").classList.add("hidden")'>Закрыть</button></div>"+html;panel.classList.remove("hidden");panel.scrollIntoView({behavior:"smooth",block:"nearest"});}
...
function renderRoleUI(){
  ...
  const items=role==="owner"?[
    ["🚀","Сигналы","Аналитика данных","signals",1],["⌁","Аналитика","Статистика системы","analysis",0],["◷","История","Раунды и события","history",0],["✦","AI-анализ","AI-инструменты","ai",0],
...
function bindSections(){
 document.querySelectorAll("[data-section]").forEach(b=>b.onclick=()=>{
   ...
   if(s==="signals"){showPanel("Сигналы",'<div class="row"><span>Режим</span><b>READ-ONLY</b></div><div class="row"><span>Источник</span><b>ОЖИДАЕТ</b></div><p class="muted">Пока источник не подключён, значения ниже являются демонстрационными.</p>'+demo.slice(-6).reverse().map((x,i)=>'<div class="row"><span>Раунд '+(i+1)+'</span><b>'+x.toFixed(2)+'×</b></div>').join(""));return;}
   if(s==="history"){showPanel("История",demo.slice().reverse().map((x,i)=>'<div class="row"><span>Раунд '+(i+1)+'</span><b>'+x.toFixed(2)+'×</b></div>').join("")+'<p class="muted">Демо-данные.</p>');return;}
   if(s==="analysis"){showPanel("Аналитика",'<div class="row"><span>Раундов в демо-наборе</span><b>'+demo.length+'</b></div><div class="row"><span>Среднее</span><b>'+((demo.reduce((a,b)=>a+b,0))/demo.length).toFixed(2)+'×</b></div><p class="muted">Это интерфейс аналитики. Реальный источник будет отображаться после подтверждённого подключения.</p>');return;}
   if(s==="ai"){showPanel("AI-анализ",'<div class="row"><span>Статус</span><b>ГОТОВ</b></div><div class="row"><span>Входные данные</span><b>ОЖИДАЮТСЯ</b></div><p class="muted">AI объясняет проверенные данные; он не выдаёт гарантии и не заменяет источник данных.</p>');return;}
