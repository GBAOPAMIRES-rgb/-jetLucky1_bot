...
async function adminScreen(type){
  ...
  if(type==="diag"){
    ...
    setScreen("Диагностика",center("🩺 Диагностика","Пошаговая read-only проверка реального источника Lucky Jet.",
      ...
      '<button type="button" class="data-row diag-row bridge-token-row" id="getBridgeToken" onclick="window.requestBridgeToken(); return false;"><span class="bridge-token-icon">🔑</span><div><b>Получить временный bridge-токен</b><small>Сгенерировать токен для browser bridge</small></div><em>›</em></button><a class="data-row diag-row bridge-helper-row" href="/luckyjet-bridge.html" target="_blank" rel="noopener"><span>🧩</span><div><b>Открыть Browser Bridge helper</b><small>Создать безопасный read-only скрипт для видимого текста</small></div><em>›</em></a>'+
      '</div>'+
